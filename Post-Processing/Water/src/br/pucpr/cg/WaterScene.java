...
public class WaterScene implements Scene {
    ...
    private float heightWater = 2.0f;
    ...
    private Matrix3f kernel = new Matrix3f(1, 2, 1,
                                           0, 0, 0,
                                           -1, -2, -1);
    ...
    public void init() {
        ...
        waterMaterial.setTexture("uWaves", new Texture(PATH + "normals/waterNormals.png"));
        //Carga do canvas para o PostFX

        canvas = MeshFactory.createCanvas();
        fb = FrameBuffer.forCurrentViewport();
        postFX = PostFXMaterial.defaultPostFX("fxBorder", fb);
    }
    ...
    public void drawWater() {
        ...
        shader.bind()
            ...
            .setUniform("uCameraPosition", camera.getPosition())
            .setUniform("uHeightWater", heightWater);
        shader.unbind();        
    ...
    public void draw() {
        ...
        shader.bind().setUniform("uKernel", kernel)
                .setUniform("uPercent", Percent).unbind();
        canvas.draw(postFX);
    }
